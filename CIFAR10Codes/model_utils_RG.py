...
def attack(model, loader, T, criterion, device,load_path,arg_softmax, image_print):
    # Evaluate the model on a dataloader with T steps for the dynamics
    ...
    eps = np.logspace(np.log10(0.05),np.log10(3),20)
    ...
    for i in range(len(eps)):
        if criterion.__class__.__name__.find('MSE')!=-1:
            art_model = PyTorchClassifier(model, torch.nn.MSELoss(), (3, 32, 32), 10, clip_values=None,preprocessing=(mean,std))
        else:
            art_model = PyTorchClassifier(model, torch.nn.CrossEntropyLoss(), (3, 32, 32), 10, clip_values=None,preprocessing=(mean,std))
        ...
        if(image_print==0):
            ...
            np.savetxt(load_path+'/Adversarial_Accuracy_UnNormalized_Retain_Grad.txt',accuracy_values,fmt='%.6f')
        else:
...
def generate(model, loader, T, criterion, device,load_path,arg_softmax):
    # Evaluate the model on a dataloader with T steps for the dynamics
    ...
    eps = np.logspace(np.log10(0.1),np.log10(3),20)
    #eps = np.array([0.5,1])
    ...
    for i in range(len(eps)):
        ...
        np.save(load_path + 'Adversarial_Images_Eps_%d.npy'%(int(eps[i]*1000)),
            cifar_adv)
        np.save(load_path + 'Labels.npy',
            labels)
    return 
def corruptions(model, loader, criterion, device, load_path, arg_softmax):
    model.eval()
    correct = 0
    start = time.time()
    corruption_accuracy = []
