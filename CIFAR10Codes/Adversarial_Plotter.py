...
def plot_adversarial_accuracyVanilla(ax):
    ...
    file_loc = [['/results/BaselineCNN/cel/Jupyter/MTModel/Adversarial_Norm_2_Accuracy_Model_hardsigmoid_Seed_1.txt'],['/results/BaselineCNN/cel/Jupyter/MTModel/Adversarial_Norm_2_Accuracy_Model_relu_Seed_1.txt']]
...
def plot_adversarial_accuracyVanillaInf(ax):
    ...
    for label_iter in range(len(labels)):
        ...
        if(len(adversarial_accuracies)!=0):
            ...
            ax.plot(adversarial_accuracy_stats[:,0],adversarial_accuracy_stats[:,1],'.-',
                                label=labels[label_iter], lw=2)
def plot_adversarial_accuracyVanillaHSJ(ax):
    max_evals = 1000
    file_loc = [['/results/BaselineCNN/cel/Jupyter/MTModel/Adversarial_HSJ_Norm_%d_Accuracy_Model_hardsigmoid_Max_Evals_%d_Seed_11.txt'],['/results/BaselineCNN/cel/Jupyter/MTModel/Adversarial_HSJ_Norm_%d_Accuracy_Model_relu_Max_Evals_%d_Seed_11.txt']]
    labels = ['VCNN (HardSigmoid)','VCNN (ReLU)']
    colors = ['g','c']
    for label_iter in range(len(labels)):
        adversarial_accuracies = []
        for file in file_loc[label_iter]:
            try:
                adversarial_accuracy = np.loadtxt(dname+file%(2,max_evals*1000))
                adversarial_accuracies.append(adversarial_accuracy)
            except OSError:
                continue 
        if(len(adversarial_accuracies)!=0):
...
def plot_adversarial_accuracyinfll(ax):
    ...
    for label_iter in range(len(labels)):
        ...
        param_list = param_list[param_list[:,0]<70]
...
def plot_adversarial_accuracy_ViT(ax,norm=2):
    file_loc = '/ViT-CIFAR/Norm_%d_Adversarial_Accuracy_Patch_%d_Augment_%d.txt'
    patches = [4,8]
    augments = [1]
    for patch in [4]]:
        for augment in augments:
            data = np.loadtxt(dname+file_loc%(norm,patch,augment))

            data = np.vstack(([0.005,data[0,1],data[0,1]],data))
            ax.plot(data[:,0],data[:,2],'.-',label='ViT Patch: %d, Aug: %d'%(patch,augment))
...
def plot_adversarial_training_accuracy_PlainCNN_MT_HSJ(ax,norm=2):
    ...
    [x.set_linewidth(1.5) for x in ax.spines.values()]
    ax.legend(loc=3,fontsize=15)
def plot_adversarial_accuracy_HSJ(ax):
            ...
            if(len(adversarial_accuracies)!=0):
                ...
                if(label_iter<2):
                    if(label_iter==0):
                        ...
                        #adversarial_accuracy_stats = np.vstack(([0.005,0.88,0],adversarial_accuracy_stats))
                        ax.errorbar(adversarial_accuracy_stats[:,0],adversarial_accuracy_stats[:,1],yerr = adversarial_accuracy_stats[:,2],
                                    label='%s'%(labels[label_iter]), lw=2,c='k')
                    else:
                        adversarial_accuracy_stats = np.vstack(([0.005,0.88,0],adversarial_accuracy_stats))
                        ax.errorbar(adversarial_accuracy_stats[:,0],adversarial_accuracy_stats[:,1], yerr = adversarial_accuracy_stats[:,2],label='%s $T_{attack}=%d$'%(labels[label_iter],params[0]),lw=2,c=colors[label_iter])
            """ if(len(adversarial_accuracies)!=0):
...
def adversarial_example_EP():
    ...
    [x.set_linewidth(1.5) for x in ax.spines.values()]
    plt.tight_layout()
    fig.savefig(dname+'/Adversarial_Accuracy2.png')
...
def adversarial_example_CNN():
    ...
    #plot_adversarial_training_accuracy_PlainCNN_MT(ax)
    plt.tight_layout()
    fig.savefig(dname+'/Adversarial_Accuracy_CNN.png')
def adversarial_example_CNN_Adv():
    ...
    plot_adversarial_training_accuracy_PlainCNN_MT(ax)
    plt.tight_layout()
    fig.savefig(dname+'/Adversarial_Accuracy_CNN_Adv.png')
def characteristic_time():
    ...
    ax.errorbar(time_mean[:,0],time_mean[:,1],yerr=time_std[:,1])
    ax.set_xscale('log')
    ax.set_xlabel(r'Free Phase Iterations',fontsize=25)
    ax.set_ylabel('Test Accuracy',fontsize=25)
    ax.xaxis.set_tick_params(labelsize=15)
    ax.yaxis.set_tick_params(labelsize=15)
    ax.set_ylim([0.05,0.95])
    ax.set_xlim([1,100])
    plt.tight_layout()
    fig.savefig('Characteristic_Time_Err.png')
...
def adversarial_HSJ():
    fig,ax = plt.subplots(1,1,figsize=(10,8))
    #plot_characteristic_time(ax[0])
    plot_adversarial_accuracy_HSJ(ax)
    plot_adversarial_accuracyVanillaHSJ(ax)
    plot_adversarial_training_accuracy_PlainCNN_MT_HSJ(ax)
    plt.tight_layout()
    fig.savefig(dname+'/HSJ_Accuracy.png')
#characteristic_time()
