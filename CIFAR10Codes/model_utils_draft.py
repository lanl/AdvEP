...
class P_CNN(torch.nn.Module):
    ...
    def Phi(self, x, y, neurons, beta, criterion):
        ...
        layers = [x] + neurons
        phi = torch.zeros(x.shape[0], device=x.device, requires_grad=True)
        ...
        if check_thm:
            for t in range(T):
             ...
             for t in range(T):
                ...
                if(T<30): #Modified for adversarial training, need to retain gradient graph but becomes memory intensive beyond 30 timesteps
                ...
                for idx in range(len(neurons)-1):
                    neurons[idx] = self.activation( grads[idx] )
                    if(T>29): #Required for EP's nudged phase
                        neurons[idx].requires_grad = True
                ...
                if(characteristic_param):
                    if not self.softmax:
                        ...
                        pred = torch.argmax(F.softmax(self.synapses[-1](neurons[-1].view(x.size(0),-1)), dim = 1), dim = 1).squeeze()
                    characteristic_time[t][1] += (y == pred).sum().item()  
...
def attack(model, loader, T, criterion, device,load_path,arg_softmax, image_print, attack_step, predict_step):
    # Evaluate the model on a dataloader with T steps for the dynamics
    global attack_param
    model.eval()
    correct = 0
    ...
    for i in range(len(eps)):
        ...
        if criterion.__class__.__name__.find('MSE')!=-1:
            ...
            art_model = PyTorchClassifier(model, torch.nn.CrossEntropyLoss(), (3, 32, 32), 10, predict_timesteps, attack_timesteps, clip_values=None,preprocessing=(mean,std))
        attack = ProjectedGradientDescentPyTorch(art_model, 2, eps[i], 2.5 * eps[i] / 20, max_iter=20, batch_size=loader.batch_size, verbose=False)   
        correct = 0
        correct_adv = 0 
        if(image_print==0):
            ...
            np.save(load_path + 'Adversarial_Images_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%(int(eps[i]*1000),attack_timesteps,predict_timesteps),
                cifar_adv)
            np.save(load_path + 'Labels_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%(int(eps[i]*1000),attack_timesteps,predict_timesteps),
                labels)
            np.save(load_path + 'Confidence_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%(int(eps[i]*1000),attack_timesteps,predict_timesteps),
                confidence)
            acc = correct/len(loader.dataset)
            ...
            np.savetxt(load_path+'/Adversarial_Accuracy_Attack_T_%d_Predict_T_%d.txt'%(attack_timesteps,predict_timesteps),accuracy_values,fmt='%.6f')
        else:
...
def hsj_attack(model, loader, T, criterion, device,load_path,arg_softmax, image_print, attack_step, predict_step):
    # Evaluate the model on a dataloader with T steps for the dynamics
    global attack_param
    model.eval()
    correct = 0
    ...
    accuracy_values = np.zeros((len(max_evals),3))
    mean = np.array([0.4914, 0.4822, 0.4465])[None,:,None,None]
    std=np.array([3*0.2023, 3*0.1994, 3*0.2010])[None,:,None,None]
    predict_timesteps = predict_step
    attack_timesteps = attack_step
    for i in range(len(max_evals)):
        ...
        if criterion.__class__.__name__.find('MSE')!=-1:
            ...
            art_model = PyTorchClassifier(model, torch.nn.CrossEntropyLoss(), (3, 32, 32), 10, predict_timesteps, attack_timesteps, clip_values=None,preprocessing=(mean,std))
        attack = HopSkipJump(art_model, norm = 2, max_eval = max_evals[i], max_iter=20, batch_size=loader.batch_size, verbose=True)   
        correct = 0
        correct_adv = 0 
        if(image_print==0):
            mbs = loader.batch_size
            ...
            labels = np.asarray(labels).flatten()
            #confidence = np.reshape(np.asarray(confidence),(-1,10))
            np.save(load_path + 'HSJ_Adversarial_Images_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%(int(max_evals[i]*1000),attack_timesteps,predict_timesteps),
                cifar_adv)
            np.save(load_path + 'HSJ_Labels_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%(int(max_evals[i]*1000),attack_timesteps,predict_timesteps),
                labels)
            ...
            acc_adv = correct_adv/len(loader.dataset)  
            print(
            f'Max Query: {max_evals[i]}; ',
            f'Train Acc: {round(acc, 2)}; ',
            f'Adversarial Acc: {round(acc_adv, 2)}; ',
            f'Time Elapsed: {time.time()-start}'
            )
            accuracy_values[i] = max_evals[i],acc,acc_adv
            np.savetxt(load_path+'/HSJ_Adversarial_Accuracy_Predict_T_%d.txt'%(predict_timesteps),accuracy_values,fmt='%.6f')            
