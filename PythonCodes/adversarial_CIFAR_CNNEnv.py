...
parser.add_argument('--mbs',type = int, default = 200, metavar = 'M', help='minibatch size')
parser.add_argument('--epochs',type = int, default = 45,metavar = 'EPT',help='Number of epochs per tasks')
...
parser.add_argument('--norm' ,type = int, default = 2, help= 'Attack Norm')
...
def adv_train_epoch(train_loader, model, optimizer, loss_fn, model_norm, train_eps):
    avg_acc = []
    avg_loss = []
    norm_layer = Normalize((0.4914, 0.4822, 0.4465), (3*0.2023, 3*0.1994, 3*0.2010))
    ...
    return model, sum(avg_acc) / len(avg_acc), sum(avg_loss) / len(avg_loss)
def adv_val_epoch(train_loader, model, optimizer, loss_fn, test_eps,norm):
...
def adv_auto_val_epoch(train_loader, model, optimizer, loss_fn, test_eps,norm):
    cifar_adv, labels, confidence = [], [], []
    avg_acc = []
    avg_loss = []
    norm_layer = Normalize((0.4914, 0.4822, 0.4465), (3*0.2023, 3*0.1994, 3*0.2010))
    ...
    attack = AutoAttack(art_model, norm = norm, eps = test_eps, batch_size=train_loader.batch_size)   
    mbs = train_loader.batch_size
    iter_per_epochs = np.ceil(len(train_loader.dataset)/mbs)
    start = time.time()
    for idx, (x, y) in enumerate(train_loader):
    ...
    return sum(avg_acc) / len(avg_acc), sum(avg_loss) / len(avg_loss),cifar_adv,labels, confidence
def adv_hsj_val_epoch(loader, model, optimizer, loss_fn, max_eval,eps, norm):
    cifar_adv, labels, confidence = [], [], []
    avg_acc = []
    avg_loss = []
    avg_predictions = []
    ...
    def predict(x):
        ...
        y_pred = to_categorical(y_pred.argmax(-1),nb_classes=10)
    ...
    base_model = BlackBoxClassifierNeuralNetwork(predict_square, input_shape=(3, 32, 32), nb_classes=10,clip_values=(0,1),preprocessing=(
            np.array([0.4914, 0.4822, 0.4465])[None, :, None, None],
            np.array([3*0.2023, 3*0.1994, 3*0.2010])[None, :, None, None]
        ))
    ...
    for idx, (x, y) in enumerate(loader):
        ...
        for i in range(1):
            x_adv = attack.generate(x.cpu().numpy())
            y_hat = torch.from_numpy(art_model.predict(x_adv)).cuda()
            y_pred = torch.from_numpy(art_model.predict(x.cpu().numpy())).cuda()
            if ((idx%(iter_per_epochs//10)==0) or (idx==iter_per_epochs-1)):
                print(
...
def train(model, train_loader, val_loader, optimizer, loss_fn, scheduler, n_epochs, model_norm= 2, adv_training=False, train_eps= 0.001):
    for epoch in range(1, n_epochs + 1):
        if adv_training:
            model, train_acc, train_loss = adv_train_epoch(train_loader, model, optimizer, loss_fn, model_norm, train_eps)
    ...
    return model
def adv_test(model, train_loader, val_loader, optimizer, loss_fn, scheduler, n_epochs, adv_training=False, train_eps = 0.001, seed=0,norm=2,model_str=''):
    if(norm==100):
    ...
    for i in range(len(test_eps)):
        ...
        val_acc, val_loss,cifar_adv, labels, confidence = adv_val_epoch(val_loader, model, optimizer, loss_fn, test_eps[i],norm)
        accuracy_values[i] = test_eps[i], train_acc, val_acc
        if(train_eps==0 and adv_training==0):
    ...
    return model
def adv_auto_test(model, train_loader, val_loader, optimizer, loss_fn, scheduler, n_epochs, adv_training=False, train_eps = 0.001, seed=0,norm=2,model_str=''):
    if(norm==100):
        test_eps = np.logspace(np.log10(0.0001),np.log10(0.1),10)  
        test_eps = np.insert(test_eps,0,1)
        test_eps = np.insert(test_eps,0,8/255)
    ...
    for i in range(len(test_eps)):
        train_acc, train_loss = val_epoch(train_loader, model, loss_fn)
        val_acc, val_loss,cifar_adv, labels, confidence = adv_auto_val_epoch(val_loader, model, optimizer, loss_fn, test_eps[i],norm)
        accuracy_values[i] = test_eps[i], train_acc, val_acc
        if(train_eps==0 and adv_training==0):
            np.save(load_path + 'AA_Adversarial_Norm_%d_Square_Images_Model_%s_Test_Eps_%d_Seed_%d.npy'%(norm_str,model_str,int(test_eps[i]*1000),seed),
                cifar_adv)
            np.save(load_path + 'AA_Labels_Norm_%d_Square_Model_%s_Test_Eps_%d_Seed_%d.npy'%(norm_str,model_str,int(test_eps[i]*1000),seed),
                labels)
            np.save(load_path + 'AA_Confidence_Norm_%d_Square_Model_%s_Test_Eps_%d_Seed_%d.npy'%(norm_str,model_str,int(test_eps[i]*1000),seed),
                confidence)
            np.savetxt(load_path+'AA_Adversarial_Norm_%d_Square_Accuracy_Model_%s_Seed_%d.txt'%(norm_str,model_str,seed),
                    accuracy_values,fmt='%.6f')
        else:
            np.save(load_path + 'AA_Adversarial_Norm_%d_Square_Images_Train_Eps_%d_Test_Eps_%d_Seed_%d.npy'%(norm_str,int(train_eps*1000),int(test_eps[i]*1000),seed),
                cifar_adv)
            np.save(load_path + 'AA_Labels_Norm_%d_Square_Train_Eps_%d_Test_Eps_%d_Seed_%d.npy'%(norm_str,int(train_eps*1000),int(test_eps[i]*1000),seed),
                labels)
            np.save(load_path + 'AA_Confidence_Norm_%d_Square_Train_Eps_%d_Test_Eps_%d_Seed_%d.npy'%(norm_str,int(train_eps*1000),int(test_eps[i]*1000),seed),
                confidence)
            np.savetxt(load_path+'AA_Adversarial_Norm_%d_Square_Accuracy_Eps_%d_Seed_%d.txt'%(norm_str,int(train_eps*1000),seed),
                    accuracy_values,fmt='%.6f')
        print(
            f'Epsilon: {test_eps[i]}; ',
            f'Train Acc: {round(train_acc, 2)}; ',
    ...
    return model
def adv_hsj_test(model, train_loader, val_loader, optimizer, loss_fn, scheduler, n_epochs, adv_training=False, train_eps = 0.001, seed=0,norm=2,model_str=''):
    max_evals = [1000]
    if(norm==100):
        test_eps = np.logspace(np.log10(0.0001),np.log10(0.1),10)  
        test_eps = np.insert(test_eps,0,8/255)
    ...
    for i in range(len(max_evals)):
        ...
        for j in range(3,len(test_eps)):
            ...
            train_acc, train_loss = val_epoch(train_loader, model, loss_fn)
            val_acc, val_loss, avg_predictions, cifar_adv, labels, confidence = adv_hsj_val_epoch(val_loader, model, optimizer, loss_fn, max_evals[i],test_eps[j],norm)
...
def dataload():
    train_dset = CIFAR10(
        ...
        transform=Compose([
            ...
            RandomCrop(size=32, padding=4, padding_mode='edge'),
            ToTensor()
        ]),
        download=True
    )
...
def adv_seed_train(seed,adv_train,norm):
    if norm == 100:
        norm_str = 100
        norm = np.inf
    ...
    for i in range(len(train_eps)):
        ...
        if norm ==2:
            ...
            torch.save(model, 
            '/vast/home/smansingh/LaborieuxEP/Equilibrium-Propagation/results/BaselineCNN/cel/Jupyter/MTModel/model_Eps_%d_Seed_%d.pt'%(int(train_eps[i]*1000),seed))
        else:
            torch.save(model, 
            '/vast/home/smansingh/LaborieuxEP/Equilibrium-Propagation/results/BaselineCNN/cel/Jupyter/MTModel/norm_%d_model_Eps_%d_Seed_%d.pt'%(norm_str,int(train_eps[i]*1000),seed))
           
def adv_seed_test(seed,adv_train,norm):
...
def adv_auto_seed_test(seed,adv_train,norm):
    ...
    for i in range(len(train_eps)):
        ...
        model = adv_auto_test(
            ...
            EPOCHS, 
            True,train_eps[i],seed,norm)
def adv_hsj_seed_test(seed,adv_train,norm):
    if norm == 100:
    ...
    for i in range(len(train_eps)):
        ...
        model = adv_hsj_test(
            ...
            EPOCHS, 
            True,train_eps[i],seed,norm)
def adv_seed_test_relu_hs(seed,norm):
...
def adv_hsj_seed_test_relu_hs(seed,norm):
    model_str = ['hardsigmoid','relu']
    for i in range(len(model_str)):
        model = torch.load('/vast/home/smansingh/LaborieuxEP/Equilibrium-Propagation/results/BaselineCNN/cel/Jupyter/MTModel/model_%s.pt'%(model_str[i]))
        optimizer = torch.optim.Adam(model.parameters(), 3e-3)
...
if args.todo == 'adv-train':
    adv_seed_train(args.seed, 1, args.norm)
if args.todo == 'train':
    adv_seed_train(args.seed, 0, args.norm)
if args.todo.find('attack')!=-1:
    if args.todo.find('hsj')!=-1:
        if args.todo.find('relu')!=-1:
            adv_hsj_seed_test(args.seed, 0, args.norm )
        else:
            adv_hsj_seed_test(args.seed, 1, args.norm)
    elif args.todo.find('auto')!=-1:
        if args.todo.find('relu')!=-1:
            adv_auto_seed_test(args.seed, 0, args.norm )
        else:
            adv_auto_seed_test(args.seed, 1, args.norm)
    else:
        if args.todo.find('relu')!=-1:
            adv_seed_test(args.seed, 0, args.norm )
        else:
            adv_seed_test(args.seed, 1, args.norm)
