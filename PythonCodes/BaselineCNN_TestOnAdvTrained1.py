...
def adv_train_epoch(train_loader, val_loader, model, optimizer, loss_fn, scheduler, train_eps, n_epochs):
    mean = np.array([0.4914, 0.4822, 0.4465])[None,:,None,None]
    std=np.array([3*0.2023, 3*0.1994, 3*0.2010])[None,:,None,None]
    art_model = PyTorchClassifier(model, loss_fn, (3, 32, 32), 10, optimizer, clip_values=None, preprocessing=(mean,std))
    attack = ProjectedGradientDescentPyTorch(art_model, 2, train_eps, 2.5 * train_eps / 20, max_iter=20, batch_size=train_loader.batch_size, verbose=False)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for x, y in train_loader:
            ...
            optimizer.step()
            scheduler.step()
        train_acc, train_loss = sum(avg_acc) / len(avg_acc), sum(avg_loss) / len(avg_loss) 
        avg_acc = []
        acg_loss = []
        for x, y in val_loader:
            ...
            loss = loss_fn(y_hat[-1], y)
            avg_loss.append(loss.item())
        val_acc, val_loss = sum(avg_acc) / len(avg_acc), sum(avg_loss) / len(avg_loss) 
    ...
    return art_model.model, accuracy_values
...
def train(model, train_loader, val_loader, optimizer, loss_fn, scheduler, n_epochs, adv_training=False, adv_testing=False, train_eps = 0):
    test_eps = [0.05,0.1,0.25,0.5,0.75,1]
    test_eps = np.logspace(np.log10(0.05),np.log10(3),20)
    test_eps = [0.05,0.5,1]
    #test_eps = np.linspace(0.05,3,20)
    
    if adv_testing == 1:
        accuracy_values = np.zeros((len(test_eps),3))
        for eps_iter in range(len(test_eps)):
            train_acc, train_loss = adv_test_epoch(val_loader, model, loss_fn, test_eps[eps_iter])
            val_acc, val_loss = adv_val_epoch(val_loader, model, loss_fn, test_eps[eps_iter])
    ...
    if adv_training == 1:
        model, accuracy_values = adv_train_epoch(train_loader, val_loader, model, optimizer, loss_fn, scheduler, train_eps, n_epochs)
        np.savetxt('/vast/home/smansingh/LaborieuxEP/Equilibrium-Propagation/results/BaselineCNN/cel/Jupyter/Training_Accuracy_Eps_%d.txt'%(int(train_eps*1000)),accuracy_values)
        torch.save(model, '/vast/home/smansingh/LaborieuxEP/Equilibrium-Propagation/results/BaselineCNN/cel/Jupyter/model_Eps_%d.pt'%(int(train_eps*1000)))
        return model
    if adv_testing==2:
        advtrain_acc, adtrain_loss = adv_test_epoch(train_loader, model, loss_fn,0.05)
        advtest_acc, advtest_loss = adv_test_epoch(val_loader, model, loss_fn,0.05)
        train_acc, train_loss = val_epoch(train_loader, model, loss_fn)
        test_acc, test_loss = val_epoch(val_loader, model, loss_fn)
        print(advtrain_acc, advtest_acc, train_acc, test_acc)
...
if(adv_testing==2):
    ...
    model = train(model, train_loader, test_loader, optimizer, loss_fn, scheduler, EPOCHS, adv_training, adv_testing)
...
if(adv_training==1):
    ...
    for eps_iter in range(len(eps)):
        ...
        optimizer = torch.optim.SGD(model.parameters(), lr=3e-4, momentum=0, 
                                    ...
                                    nesterov=False)
        EPOCHS = 40
        scheduler = OneCycleLR(optimizer=optimizer, max_lr=1, epochs=EPOCHS, steps_per_epoch=len(train_dset) // BATCH_SIZE, div_factor=1e1,
        final_div_factor=1e2,  three_phase=True)
        model.to(device)
        art_model = train(model, 
        train_loader, 
        test_loader, 
