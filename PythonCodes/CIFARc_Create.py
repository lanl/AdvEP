...
def frost(x, severity=1):
    ...
    frost = cv2.imread(os.getcwd()+'/'+filename)
    print(frost.shape)
...
test_data = dset.CIFAR10('vision-greg/cifarpy', train=False,download=True)
convert_img = trn.Compose([trn.ToTensor(),trn.Normalize(mean=(0.4914, 0.4822, 0.4465), 
                                                                                      std=(3*0.2023, 3*0.1994, 3*0.2010)), trn.ToPILImage()])

#print(test_data.batch_size)
for method_name in d.keys():
    if(os.path.exists('vision-greg2/CIFAR-10-C/' + d[method_name].__name__ + '.npy')):
        continue
    ...
    np.save('vision-greg2/CIFAR-10-C/' + d[method_name].__name__ + '.npy',
            np.array(cifar_c).astype(np.uint8))

    np.save('vision-greg2/datasets/CIFAR-10-C/labels.npy',
