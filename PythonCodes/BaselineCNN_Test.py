...
def adv_test_epoch(train_loader, model, optimizer, loss_fn, scheduler,eps):
    avg_acc = []
    avg_loss = []
    ...
    for x, y in train_loader:
        ...
        y_hat = torch.from_numpy(art_model.predict2(torch.from_numpy(x_adv))) 
...
def adv_val_epoch(train_loader, model, loss_fn,eps):
    ...
    for x, y in train_loader:
        ...
        x_adv = attack.generate(x.cpu().numpy())
        y_hat = torch.from_numpy(art_model.predict2(x_adv)).to(device)
...
def val_epoch(val_loader, model, loss_fn):
    ...
    return sum(avg_acc) / len(avg_acc), sum(avg_loss) / len(avg_loss)
def train(model, train_loader, val_loader, optimizer, loss_fn, scheduler, n_epochs, adv_training=False, adv_testing=False, epsi = 0):
    ...
    if adv_testing == 1:
        ...
        for epsi in range(len(eps)):
            model, train_acc, train_loss = adv_test_epoch(test_loader, model, optimizer, loss_fn, scheduler,eps[epsi])
            val_acc, val_loss = val_epoch(train_loader, model, loss_fn)
            accuracy_values[epsi] = eps[epsi], val_acc, train_acc
            print(eps[epsi], val_acc, train_acc)
        np.savetxt('/vast/home/smansingh/LaborieuxEP/Equilibrium-Propagation/results/BaselineCNN/cel/Jupyter/Adversarial_Accuracy_UnNormalized_HardSigmoid.txt',accuracy_values,fmt='%.6f')    
    ...
    if adv_training == 1:
        accuracy_values = np.zeros((n_epochs,2))
        for epoch in range(1, n_epochs + 1):
            model, train_acc, train_loss = adv_train_epoch(train_loader, model, optimizer, loss_fn, scheduler,eps[epsi])
            val_acc, val_loss = 0, 0
            #val_acc, val_loss = adv_val_epoch(val_loader, model, loss_fn,eps[epsi])
            accuracy_values[epoch-1] = epoch, train_acc
            print(
            f'Epoch: {epoch}; ',
            f'Epsilon: {eps[epsi]}'
...
adv_training = 1
adv_testing = 0
transform_train, transform_test = DataLoaderTransforms(adv_training)
train_dset = CIFAR10(
            './data/cifar',
            train=True,
