...
def adv_auto_val_epoch(train_loader, model, optimizer, loss_fn,mean, std, test_eps,norm):
    cifar_adv, labels, confidence = [], [], []
    avg_acc = []
    avg_loss = []
    norm_layer = Normalize(mean, std)
...
def adv_test(model, train_loader, val_loader, optimizer, loss_fn, mean, std, scheduler, n_epochs, adv_training=False, train_eps = 0.001, seed=0,norm=2,model_str=''):
    ...
    return model
def adv_auto_test(model, train_loader, val_loader, optimizer, loss_fn, mean, std, scheduler, n_epochs, adv_training=False, train_eps = 0.001, seed=0,norm=2,model_str=''):
    if(norm==100):
        test_eps = np.logspace(np.log10(0.0001),np.log10(0.1),10)  
        test_eps = np.insert(test_eps,0,1)
        test_eps = np.insert(test_eps,0,8/255)
    ...
    for i in range(len(test_eps)):
        train_acc, train_loss = val_epoch(train_loader, model, loss_fn,mean, std)
        val_acc, val_loss,cifar_adv, labels, confidence = adv_auto_val_epoch(val_loader, model, optimizer, loss_fn, mean, std, test_eps[i],norm)
        accuracy_values[i] = test_eps[i], train_acc, val_acc
...
def adv_hsj_seed_test(seed,adv_train,norm):
    ...
    for i in range(len(train_eps)):
        ...
        model = adv_hsj_test(
            ...
            EPOCHS, 
            True,train_eps[i],seed,norm)
def adv_seed_test_relu_hs(seed,adv_train,norm):
...
def adv_hsj_seed_test_relu_hs(seed,adv_train,norm):
    model_str = ['hardsigmoid','relu']
