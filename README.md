...
|332|output=model_outputs[-1]|neurons = self._model(torch.from_numpy(x_preprocessed[begin:end]).to(self._device),y, neurons, self.T1, 0.0,self._loss,check_thm=False)[-1]| Use T1 free-phase iterations for obtaining predictions
