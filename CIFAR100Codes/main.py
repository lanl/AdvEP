...
if args.save:
    ...
    if args.load_path=='':
        if args.todo =='advtrain' or args.todo=='attack':
            path = 'results/'+args.alg+'_adv/'+args.loss+'/'+date+'/'+time+'_gpu'+str(args.device)
        else:
...
if args.task=='MNIST':
    ...
    train_loader = torch.utils.data.DataLoader(cifar100_train_dset, batch_size=mbs, shuffle=True, num_workers=1)
    test_loader = torch.utils.data.DataLoader(cifar100_test_dset, batch_size=200, shuffle=False, num_workers=1)
...
if args.load_path=='':
    ...
    if args.model=='MLP':
        ...
        elif args.task=='CIFAR100' or args.task=='CIFAR10':    
           pools = make_pools(args.pools)
           unpools = make_unpools(args.pools)
           channels = [3]+args.channels
           if args.model=='CNN':
                model = P_CNN(32, channels, args.kernels, args.strides, args.fc, pools, unpools, args.paddings,
                              activation=activation, softmax=args.softmax)
           elif args.model=='VFCNN':
    ...
    if args.todo == 'advtrain':
        model = torch.load(args.load_path + '/model_Train_Eps_%d.pt'%(int(args.eps*1000)), map_location=device)
    else:
        try: 
            model = torch.load(args.load_path + '/model_Train_Eps_%d.pt'%(int(args.eps*1000)), map_location=device)
...
model.to(device)
neurons = model.init_neurons(100,device)
